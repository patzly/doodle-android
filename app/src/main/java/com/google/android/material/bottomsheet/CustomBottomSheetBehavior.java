...
import com.google.android.material.internal.CustomViewUtils;
 ...
 * window-like. For BottomSheetDialog use {@link CustomBottomSheetDialog#setTitle(int)}, and for
...
public class CustomBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
  ...
  public CustomBottomSheetBehavior(@NonNull Context context, @Nullable AttributeSet attrs) {
    ...
    if (a.hasValue(R.styleable.BottomSheetBehavior_Layout_backgroundTint)) {
      this.backgroundTint = getColorStateList(
   ...
   * This method should be called before {@link CustomBottomSheetDialog#show()} in order for the width to
   ...
   * CustomBottomSheetDialog#show()} in order for the height to be adjusted as expected.
   *
  ...
  private void setWindowInsetsListener(@NonNull View child) {
    ...
    CustomViewUtils.doOnApplyWindowInsets(
        child,
        (view, insets, initialPadding) -> {
          ...
          boolean isRtl = CustomViewUtils.isLayoutRtl(view);
