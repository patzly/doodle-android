...
import com.plattysoft.leonids.initializers.SpeedByComponentsInitializer;
...
public class ParticleSystem {
   ...
   */
  public ParticleSystem(ViewGroup parentView, int maxParticles, Drawable drawable,
      long timeToLive) {
  ...
  public ParticleSystem setSpeedRange(float speedMin, float speedMax) {
    mInitializers
        .add(new SpeedModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), 0, 360));
    return this;
  }

  /**
   * Initializes the speed range and angle range of emitted particles. Angles are in degrees and non
   * negative: 0 meaning to the right, 90 to the bottom,... in clockwise orientation. Speed is non
  ...
  public ParticleSystem setSpeedModuleAndAngleRange(float speedMin, float speedMax, int minAngle,
    ...
    mInitializers.add(
        new SpeedModuleAndRangeInitializer(dpToPx(speedMin), dpToPx(speedMax), minAngle, maxAngle));
    return this;
  }

  /**
   * Initializes the speed components ranges that particles will be emitted. Speeds are measured in
   * density pixels per millisecond.
   *
   ...
   * @return This.
   */
  public ParticleSystem setSpeedByComponentsRange(float speedMinX, float speedMaxX, float speedMinY,
      float speedMaxY) {
  ...
  /**
   * Initializes the initial rotation range of emitted particles. The rotation angle is measured in
   * degrees with 0° being no rotation at all and 90° tilting the image to the right.
   *
  ...
  /**
   * Initializes the scale range of emitted particles. Will scale the images around their center
   * multiplied with the given scaling factor.
   *
  ...
  /**
   * Initializes the rotation speed of emitted particles. Rotation speed is measured in degrees per
   * second.
   *
  ...
  /**
   * Initializes the rotation speed range for emitted particles. The rotation speed is measured in
   * degrees per second and can be positive or negative.
   *
   ...
   * components in x and y direction are controlled by the acceleration angle. The acceleration is
   * measured in density pixels per square millisecond. The angle is measured in degrees with 0°
   * pointing to the right and going clockwise.
   */
   ...
   * measured in pixels per square millisecond. The angle is measured in degrees with 0° meaning to
   * the right and orientation being clockwise. The angle controls the acceleration direction.
   *
  ...
  public ParticleSystem setFadeOut(long millisecondsBeforeEnd, Interpolator interpolator) {
    mModifiers.add(
        new AlphaModifier(255, 0, mTimeToLive - millisecondsBeforeEnd, mTimeToLive, interpolator));
    return this;
  }
   ...
   * amount of particles available on create no new particles will be created
   *
   ...
   * @param particlesPerSecond Number of particles per second that will be emitted (evenly
   *                           distributed)
  ...
  private void startAnimator(Interpolator interpolator, long animationTime) {
    ...
    mAnimator.addUpdateListener(animation -> onUpdate((Integer) animation.getAnimatedValue()));
  ...
  /**
   * Stops emitting new particles, but will draw the existing ones until their timeToLive expire For
   * an cancellation and stop drawing of the particles, use cancel instead.
   */
  ...
  /**
   * Cancels the particle system and all the animations. To stop emitting but animate until the end,
   * use stopEmitting instead.
   */
  public void cancel() {
