...
/*
 * This class has been extracted from the android source code. In the latest versions, most of the
 * code has been moved for native processing. This is pure java implementation that was used before
 * the native integration. This works very good in terms of performance. The link to the file :
...
public class PathParser {
  ...
  static final String TAG = PathParser.class.getSimpleName();
      ...
      for (int k = 0; k < val.length; k += incr) {
        switch (cmd) {
          ...
          case 'L': // lineto - Draw a line from the current point
            if ((previousCmd == 'M' || previousCmd == 'm') && val[k] == currentX
                && val[k + 1] == currentY) {
              path.addCircle(val[k], val[k + 1], 1, Path.Direction.CW);
            } else {
              path.lineTo(val[k], val[k + 1]);
